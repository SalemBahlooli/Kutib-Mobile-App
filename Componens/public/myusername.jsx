...
const MyuserName = ({clerk_id  ,  fontSize , lefticon , righticon  , style}) => {
    ...
    <React.Fragment>
      ...
      {righticon === true&& (
        <View style={{marginRight:5 , marginTop:4 }}>
          ...
          <ShieldCheck size={10} color={colors.Dark_primary} />

        )}

        {users[0].Writer && (
          // <Ionicons name="book" size={15} color="#14b0ff" />
          <PenTool size={16} color="#14b0ff" />
        )}
  ...
  {users[0].name}
        ...
        {lefticon === true&& (
         <View style={{marginLeft:5 , marginTop:4,  flexDirection:'row'}}>
           {users[0].Admin && (
          <ShieldCheck size={16} color='#a042ff' />
        )}
